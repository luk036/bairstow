...
const PI: _ = pi;
fn horner_backward(pb: List, n: i32, alpha: FoC) -> FoC {
    // "[summary]
    //
    //     Args:
...
fn initial_aberth(pa: Vec<FoC>) -> Vec<complex> {
    // "[summary]
    //
    //     Args:
    //         pa (List): [description]
    ...
    //     ";
    let N: int = (pa.len() - 1);
    let c: FoC = (-(pa[1]) / (N * pa[0]));
    let Pc: FoC = horner_eval(pa.copy(), N, c);
    let re: FoC = -(Pc).pow((1.0 / N));
...
fn initial_aberth_orig(pa: Vec<FoC>) -> Vec<complex> {
    // "[summary]
    //
    //     Args:
    //         pa (List): [description]
...
fn aberth<T0>(pa: Vec<FoC>, zs: Vec<complex>, options: T0) -> (Vec<complex>, i32, bool) {
